...
export default {
  ...
  methods:{
    ...
    getCollageInfo(){
      ...
      let act_id = this.$common.VL.getRequest().aid || 3;
    ...
    toOrder(type,ne){
      ...
      let act_id = this.$common.VL.getRequest().aid || 3;
      let source_id = this.$common.VL.getRequest().sid || 107;
      if(!ph){
    ...
    shareConfig(){
      ...
      if(vid > 0){
        if(this.haveGroup){
          if(this.isSelf){
            link = window.location.origin + '/mini_pay/mc/discount.html?view_member_id=' + member_id
          }else {
            link = window.location.origin + '/mini_pay/mc/discount.html?view_member_id=' + vid
          }
        }else {
          link = window.location.origin + '/mini_pay/mc/discount.html?view_member_id=' + member_id
        }
