      ...
      if(this.loginStatus){
        ...
        this.$axios.post(actionUrl,fd,conf).then((res) => {
          if(res.data.code == 200){
            ...
            if(vid > 0 ){
              _this.$router.push({name:'Order',query:{type:type,vid:vid,aid:act_id,sid:source_id}})
            }else {
              _this.$router.push({name:'Order',query:{type:type,aid:act_id,sid:source_id}})
            }
          }else{
        ...
        _hmt.push(['_trackEvent', '按钮', '点击购买', '登录页']);
      }
    },
    blur(){
      this.blurFlag = true;
