  ...
  <div>
    <div class="collage">
      <div :class="shareStatus||sucTagStatus?'container blur':'container'">
        <img src="https://qnfile.abctime.com/abc_h5/collage_e/img_hd_py.png" class="banner">
        <div class="time_box" v-show="!hideTimeout">
           距结束还剩<span>{{hour}}:{{minute}}:{{sister}}</span>
        </div>
        <div class="content_box">
          <p class="title">《拼音来啦》AI拼音动画课</p>
          <div class="info">
            <div class="price">¥{{groupPrice}}<span>¥199</span></div>
            <div class="num"><span>{{suc_num}}</span>人已成团</div>
          </div>
          <p class="desc">16讲拼音互动动画课，不但让孩子在游戏中学会正确发音和拼读，还能掌握150个常见词汇，真正学会运用~</p>
        </div>
        ...
        <img src="https://qnfile.abctime.com/abc_h5/collage_e/img_content_py.png" class="info_pic">
        ...
        <div v-show="!selfSuc && selfNew && !defaultStatus && !endStatus" class="btn_box2" @click="toOrder(2,1)">发起新团<span class="icon"></span></div>
        <div v-show="selfSuc && !defaultStatus && !endStatus" class="btn_box2" @click="toDown">{{successTxt}}<span class="icon"></span></div>
      </div>
    </div>
...
export default {
  ...
  methods:{
    ...
    getCollageInfo(){
      ...
      this.$axios.get(url+'?'+fd).then((res) => {
        if(res.data.code == 200){
          ...
          _this.suc_num = res.data.data.join_num
        }
      })
    },
    getMemberInfo(){
    ...
    backCollage(){
      window.location.href = window.location.origin + '/mk/collagePy/'
    },
    toOrder(type,ne){
      if(ne){
        window.location.href = window.location.origin + '/mk/collagePy/'
      ...
      let source_id = this.$common.VL.getRequest().source_id || '';
      if(!ph){
        if(this.view_member_id > 0 ){
          this.$router.push({name:'Login',query:{type:type,vid:this.view_member_id}})
        }else {
      ...
      if(type>0){
        if(this.view_member_id > 0 && !ne){
          this.$router.push({name:'Order',query:{type:type,vid:this.view_member_id}})
        }else {
          this.$router.push({name:'Order',query:{type:type}})
        }
        if(type == 1){
        }else if(type == 2){
        }
    ...
    shareConfig(){
      ...
      let configObj = {
        title:'《拼音来啦》 3人拼团价49元',
        desc: '16讲拼音互动课，让孩子在游戏中学会正确发音和拼读，还能掌握150个常见词汇，学会运用~',
        imgUrl: 'https://qnfile.abctime.com/share_logo_py.jpg',
      };
      let vid = this.$common.VL.getRequest().view_member_id
      let sid = this.$common.VL.getRequest().source_id
      if(vid > 0){
        if(this.haveGroup){
          if(this.isSelf){
            link = window.location.origin + window.location.pathname + '?view_member_id=' + member_id
          }else {
            link = window.location.origin + window.location.pathname + '?view_member_id=' + vid
          }
        }else {
          link = window.location.origin + window.location.pathname + '?view_member_id=' + member_id
        }
