<template>
  <div class="catalog">
    <header>
      <img :src="lessonInfo.banner"/>
      <div class="info_hd">
          <p class="name">{{lessonInfo.name}}</p>
          <p class="desc">{{lessonInfo.desc}}</p>
    ...
    data () {
      return {
        lessonInfo:{
            // name: '美丽的地球',
            // desc: '足不出户，带孩子领略地球之美！',
            // banner: 'https://qnfile.abctime.com/extend/list_lesson4.png'        
        },
        cataArr:[],
        courseArr:[],
        nickname:'',
      ...
      getCourseList(){
        ...
        this.$axios.get(wUrl+'?'+fd).then((res) => {
          ...
          if(res.data.code == 200){
            this.lessonInfo = res.data.data.course_info
