  ...
  export default {
    ...
    methods: {
      ...
      getCourseInfo(flag){
        ...
        let fd = this.$common.getParam('get', {member_id: member_id,lesson_id:lesson_id});
      ...
      activeLesson(lid){
        ...
        let fd = this.$common.getParam('get', {member_id: member_id,lesson_id:lesson_id,location:lid});
        this.$axios.get(wUrl+'?'+fd).then((res) => {
          if(res.data.code == 200){
          }
        })
