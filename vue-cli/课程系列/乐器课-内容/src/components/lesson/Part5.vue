  ...
  export default {
    ...
    methods:{
      next(event){
        ...
        localStorage.setItem('musicalPartCover',this.next_cover);
      ...
      getCourseList(){
        ...
        this.$axios.get(wUrl+'?'+fd).then((res) => {
          ...
          for(let i in this.courseArr){
            if(!this.courseArr[i].is_finish){
              let les = 'extendAbcFinish'+this.courseArr[i].id;
              if(!localStorage.getItem(les)){
                this.showFinish = true;
                this.next_id = parseInt(this.courseArr[i].id);
                this.next_cover = this.courseArr[i].cover
                isCompleted = false
                // localStorage.setItem(les,true)
              }
              break
